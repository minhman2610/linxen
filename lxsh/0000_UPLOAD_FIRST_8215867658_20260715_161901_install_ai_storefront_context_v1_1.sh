...
PATCH_NAME="install_ai_storefront_context_v1_1"
ROOT="$(pwd)"
TARGET="app/Console/Commands/Tools/AiStorefrontContextCommand.php"
OPTIONAL_PATCH_CONTEXT="app/Console/Commands/Tools/AiPatchContextCommand.php"
...
rollback() {
    ...
    printf '%s
' 'Patch failed. Rolling back changed files.' >&2

    if [ "$TARGET_EXISTED" -eq 1 ] && [ -f "$BACKUP_DIR/$TARGET" ]; then
        mkdir -p "$(dirname "$TARGET")"
        cp "$BACKUP_DIR/$TARGET" "$TARGET"
    elif [ "$TARGET_EXISTED" -eq 0 ]; then
        rm -f "$TARGET"
    fi

    if [ "$OPTIONAL_EXISTED" -eq 1 ] && [ -f "$BACKUP_DIR/$OPTIONAL_PATCH_CONTEXT" ]; then
        mkdir -p "$(dirname "$OPTIONAL_PATCH_CONTEXT")"
        cp "$BACKUP_DIR/$OPTIONAL_PATCH_CONTEXT" "$OPTIONAL_PATCH_CONTEXT"
    fi
...
if [ ! -f "$ROOT/artisan" ]; then
    printf '%s
' 'Run this patch from the Laravel project root. File artisan was not found.' >&2
    exit 1
fi

if ! command -v php >/dev/null 2>&1; then
    printf '%s
' 'PHP CLI was not found.' >&2
    exit 1
fi
...
if [ -f "$OPTIONAL_PATCH_CONTEXT" ]; then
    OPTIONAL_EXISTED=1
    mkdir -p "$BACKUP_DIR/$(dirname "$OPTIONAL_PATCH_CONTEXT")"
    cp "$OPTIONAL_PATCH_CONTEXT" "$BACKUP_DIR/$OPTIONAL_PATCH_CONTEXT"
fi

mkdir -p "$(dirname "$TARGET")"
TMP_NEW="$(mktemp "${TMPDIR:-/tmp}/ai_storefront_context.XXXXXX")"
cat > "$TMP_NEW" <<'PHPFILE'
<?php
...
if [ -f "$OPTIONAL_PATCH_CONTEXT" ]; then
    TMP_OLD="$(mktemp "${TMPDIR:-/tmp}/ai_patch_context_harden.XXXXXX")"
    cp "$OPTIONAL_PATCH_CONTEXT" "$TMP_OLD"

    TMP_PATCHER="$(mktemp "${TMPDIR:-/tmp}/ai_patch_context_patcher.XXXXXX")"
    cat > "$TMP_PATCHER" <<'PHPPATCHER'
<?php
$path = $argv[1] ?? '';
if ($path === '' || !is_file($path)) {
    fwrite(STDERR, "Optional AiPatchContextCommand not found; skipping hardening.\n");
    exit(0);
}
...
if (str_contains($source, 'AI_PATCH_CONTEXT_RECURSIVE_REDACTION_V2')) {
    exit(0);
}
$replacements = [
    "'schema_json', 'validation_schema', 'rules_json', 'options_json', 'json', 'extra', 'snapshot'," => "'schema_json', 'validation_schema', 'rules_json', 'options_json', 'json', 'extra', 'snapshot',\n                'raw_data', 'raw_json', 'raw_payload',",
    'return $this->truncateDeep($decoded);' => 'return $this->redactDeepForContext($decoded);',
    'return $this->truncateDeep($value);' => 'return $this->redactDeepForContext($value);',
    'return $this->truncateDeep((array) $value);' => 'return $this->redactDeepForContext((array) $value);',
];
foreach ($replacements as $from => $to) {
    if (!str_contains($source, $from)) {
        fwrite(STDERR, "Required hardening anchor not found: {$from}\n");
        exit(1);
    }
    $source = str_replace($from, $to, $source);
}
$anchor = '    protected function truncateDeep(mixed $value, int $depth = 0): mixed';
if (!str_contains($source, $anchor)) {
    fwrite(STDERR, "truncateDeep anchor not found.\n");
    exit(1);
}
$methods = <<<'METHODS'
    /**
     * AI_PATCH_CONTEXT_RECURSIVE_REDACTION_V2
     *
     * Redact secrets/PII recursively, including JSON nested inside raw_data.
     */
    protected function redactDeepForContext(mixed $value, int $depth = 0, string $path = ''): mixed
    {
        if ($depth > 8) {
            return '/* truncated: max depth */';
        }
        ...
        if (is_array($value)) {
            ...
            foreach ($value as $key => $item) {
                ...
                if ($this->shouldRedactNestedKey($nextPath)) {
                    $result[$key] = '***REDACTED***';
                    continue;
                }

                $result[$key] = $this->redactDeepForContext($item, $depth + 1, $nextPath);
            }
        ...
        if (is_string($value)) {
            ...
            if ($trimmed !== '' && in_array($trimmed[0], ['{', '['], true)) {
                ...
                if (json_last_error() === JSON_ERROR_NONE) {
                    return $this->redactDeepForContext($decoded, $depth + 1, $path);
                }
            }

            return $this->truncateStringForContext($this->redactInlineSecrets($value));
        }

        return $value;
    }

    protected function shouldRedactNestedKey(string $key): bool
    {
        $key = Str::lower($key);
        ...
        return Str::contains($key, [
            'password', 'passwd', 'secret', 'token', 'api_key', 'apikey', 'access_key',
            'private_key', 'credential', 'client_secret', 'bearer', 'authorization',
            'tfa', 'otp_secret', 'cookie', 'session_id', 'remember_token',
            'refresh_token', 'access_token',
            'phone', 'mobile', 'email', 'address', 'birthday', 'contact_number',
            'receiver_phone',
        ]);
    }

    protected function redactInlineSecrets(string $value): string
    {
        $patterns = [
            '/(Authorization\s*[:=]\s*Bearer\s+)[A-Za-z0-9._~+\/-]+/i' => '$1***REDACTED***',
            '/(Bearer\s+)[A-Za-z0-9._~+\/-]{20,}/i' => '$1***REDACTED***',
...
METHODS;
$source = str_replace($anchor, $methods . $anchor, $source);

$old = <<<'OLD'
    protected function shouldRedactColumn(string $column, string $settingKey = ''): bool
    {
        $haystack = Str::lower($column . ' ' . $settingKey);
...
if (!str_contains($source, $old)) {
    fwrite(STDERR, "shouldRedactColumn anchor not found.\n");
    exit(1);
}
...
PHPPATCHER
    ...
    php "$TMP_PATCHER" "$TMP_OLD"
    php -l "$TMP_OLD"
    mv "$TMP_OLD" "$OPTIONAL_PATCH_CONTEXT"
    chmod 0644 "$OPTIONAL_PATCH_CONTEXT"
    rm -f "$TMP_PATCHER"
fi

php -l "$TARGET"

# AI_STOREFRONT_CONTEXT_V1_1_LOCAL_DB_SAFE_CACHE_CLEAR
# optimize:clear includes cache:clear. On local projects whose cache store is
# database/SQLite, the command can fail simply because the local DB file is
# intentionally absent. Cache clearing is therefore optional and must never
# roll back a syntactically valid command installation.
if ! php artisan optimize:clear; then
    printf '%s\n' 'Warning: php artisan optimize:clear failed; continuing because cache/runtime checks are optional on local.' >&2
    printf '%s\n' 'This commonly occurs when CACHE_STORE/CACHE_DRIVER uses database and the local SQLite file does not exist.' >&2

    # Best-effort file-backed clears. Every command remains optional because
    # Laravel versions and local bootstrap environments differ.
...
if ! php artisan list --raw | grep -q '^tools:ai-storefront-context'; then
    printf '%s
' 'Command was written but Laravel did not discover tools:ai-storefront-context.' >&2
    printf '%s
' 'Check app/Console/Kernel.php command loading for this source.' >&2
    exit 1
fi

SMOKE_OUT="$(mktemp "${TMPDIR:-/tmp}/ai_storefront_context_smoke.XXXXXX")"
if ! php artisan tools:ai-storefront-context     --mode=audit-v1     --site=linxen     --project-id=smoke-current-source     --scan=routes,pages     --max-scan-files=300     --max-files=30     --preview     --no-db     > "$SMOKE_OUT" 2>&1; then
    cat "$SMOKE_OUT" >&2
    rm -f "$SMOKE_OUT"
    exit 1
fi
...
printf '%s
' 'AI Storefront Context command installed successfully.'
printf '%s
' "Laravel root: $ROOT"
printf '%s
' "Created/updated: $TARGET"
if [ "$OPTIONAL_EXISTED" -eq 1 ]; then
    printf '%s
' "Hardened recursive secret/PII redaction: $OPTIONAL_PATCH_CONTEXT"
fi
printf '%s
' "Backup: $BACKUP_DIR"
printf '%s
' ''
printf '%s
' 'Run in Lin Xen V1 source:'
printf '%s
' 'php artisan tools:ai-storefront-context --mode=audit-v1 --site=linxen --project-id=linxen-v1 --url=https://linxen.vn --download'
printf '%s
' ''
printf '%s
' 'Run in ERP source:'
printf '%s
' 'php artisan tools:ai-storefront-context --mode=erp-contract --site=linxen --project-id=3mg-erp --route-prefix=api/storefront --tables=storefronts --download'
